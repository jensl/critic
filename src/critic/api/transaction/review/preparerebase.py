...
from ..rebase import ModifyRebase
...
async def prepare_rebase(
            ...
            if not gitaccess.SHA1_PATTERN.match(new_upstream_ref):
                async with api.critic.Query[str](
                    ...
                    try:
                        ...
                        raise api.log.rebase.Error("Specified new_upstream is invalid.")
            try:
    ...
    return ModifyRebase.create(transaction, review, old_upstream, new_upstream, branch)
