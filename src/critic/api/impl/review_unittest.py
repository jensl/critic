...
def basic():
    ...
    assert all(
        isinstance(review_filter, api.reviewfilter.ReviewFilter)
    ...
    assert review.filters[2].id not in (review.filters[0].id, review.filters[1].id)
    ...
    assert all(isinstance(commit, api.commit.Commit) for commit in review.commits)
