...
def basic(arguments):
    ...
    updater = api.user.fetch(critic, name=arguments.updater_name)
    ...
    for index, (update, expected_update) in enumerate(zip(updates, expected_updates)):
        ...
        for update_commit, expected_sha1 in zip(
            ...
            assert update_commit.sha1 == expected_sha1, "added: %s != %s" % (
                update_commit,
                expected_sha1,
            )
        ...
        for update_commit, expected_sha1 in zip(
            ...
            assert update_commit.sha1 == expected_sha1, "removed: %s != %s" % (
                update_commit,
                expected_sha1,
            )
