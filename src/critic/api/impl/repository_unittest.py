...
def basic(arguments):
    ...
    annotated_tag = repository.resolveRef(
        "007-repository/annotated-tag", expect="commit"
    )
    assert annotated_tag == head.sha1
    ...
    commits = repository.listCommits(
        (commit for commit in [commit0]),
        (commit for commit in [commit5]),
        args=["--merges"],
    )
    assert len(commits) == 0

    commits = repository.listCommits(args=["--reverse", "%s..%s" % (commit5, commit0)])
    assert len(commits) == 5
    ...
    except api.repository.InvalidId:
        pass
