...
def basic(arguments):
    ...
    extra = api.user.fetch(critic, name="__disabled_7__")

    all_users = [admin, alice, bob, dave, erin, howard, carol, felix, gina, iris, extra]
    ...
    assert alice.primary_emails[0].verified == "trusted"
    ...
    assert isinstance(filters[0], api.repositoryfilter.RepositoryFilter)
    ...
    assert all(isinstance(delegate, api.user.User) for delegate in filters[0].delegates)
    ...
    except api.user.InvalidId as error:
    ...
    try:
        api.user.fetchMany(critic, user_ids=[alice.id], names=[alice.name])
    except AssertionError:
        pass
    else:
        assert False
    ...
    try:
        ...
        assert error.args[0] == (
            "Invalid user names: %r" % ["nobody", "anybody"]
        ), error.args[0]
    ...
    assert isinstance(users, list)
    assert users == sorted(
        [user for user in all_users if user.status == "current"],
    ...
    assert isinstance(users, list)
    assert users == sorted(
        [user for user in all_users if user.status in ("current", "absent")],
        key=lambda user: user.id,
    )
    ...
    assert isinstance(users, list)
    assert users == sorted(
        [user for user in all_users if user.status in ("retired", "absent")],
        key=lambda user: user.id,
    )
...
def preferences():
    ...
    compactMode = alice.getPreference("commit.diff.compactMode", repository=repository)
    assert compactMode.item == "commit.diff.compactMode"
    assert compactMode.value is True
    assert compactMode.user is None
    assert compactMode.repository is None
    ...
    visualTabs = alice.getPreference("commit.diff.visualTabs", repository=repository)
    assert visualTabs.value is True
    assert visualTabs.user is alice
    assert visualTabs.repository is None
    ...
    expandAllFiles = alice.getPreference("commit.expandAllFiles", repository=repository)
