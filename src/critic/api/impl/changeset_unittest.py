...
CUSTOM_PATHLIST = frozenset(
    [
        "src/auth/accesscontrol.py",
...
SINGLE_PATHLIST = frozenset(
    ["testing/__init__.py", "testing/repository.py", "testing/virtualbox.py"]
)

ROOT_PATHLIST = frozenset(
    [
        ".gitignore",
        "CONTRIBUTORS",
        "COPYING",
...
def is_empty_changeset(changeset, changeset_type):
    return (
        changeset.id is None
        and changeset.type == changeset_type
        and changeset.files is None
    )


def check_types(changeset):
    return (
        isinstance(changeset.id, int)
        and isinstance(changeset.type, str)
        and isinstance(changeset.files, list)
    )


async def custom_changeset(phase, api, critic, repository):
    if phase == "pre":
        from_commit = await api.commit.fetch(repository, sha1=FROM_SHA1)
        to_commit = await api.commit.fetch(repository, sha1=TO_SHA1)

        try:
            await api.changeset.fetch(
                repository, from_commit=from_commit, to_commit=to_commit
            )
        ...
        from_commit = await api.commit.fetch(repository, sha1=FROM_SHA1)
        to_commit = await api.commit.fetch(repository, sha1=TO_SHA1)
        custom_changeset = await api.changeset.fetch(
            repository, from_commit=from_commit, to_commit=to_commit
        )
        assert (
            check_types(custom_changeset) and custom_changeset.type == "custom"
        ), "custom_changeset has incorrect types"

        paths = frozenset(
            [filechange.file.path for filechange in custom_changeset.files]
        )

        assert (
            paths == CUSTOM_PATHLIST
...
async def direct_changeset(phase, api, critic, repository):
    if phase == "pre":
        single_commit = await api.commit.fetch(repository, sha1=TO_SHA1)
        from_single_commit = await api.commit.fetch(repository, sha1=FROM_SINGLE_SHA1)
        try:
            await api.changeset.fetch(repository, single_commit=single_commit)
        except api.changeset.Delayed:
            pass

        try:
            await api.changeset.fetch(
                repository, from_commit=from_single_commit, to_commit=single_commit
            )
        except api.changeset.Delayed:
            pass
        ...
        try:
            await api.changeset.fetch(
                repository, from_commit=single_commit, to_commit=from_single_commit
            )
        except api.changeset.Delayed:
            pass
        ...
        single_commit = await api.commit.fetch(repository, sha1=TO_SHA1)
        from_single_commit = await api.commit.fetch(repository, sha1=FROM_SINGLE_SHA1)
        changeset = await api.changeset.fetch(repository, single_commit=single_commit)
        equiv_changeset = await api.changeset.fetch(
            repository, from_commit=from_single_commit, to_commit=single_commit
        )

        assert (
            changeset.id == equiv_changeset.id
        ), "changeset and equiv_changeset have different ids"

        assert (
            changeset.type == equiv_changeset.type
        ...
        changeset_files = frozenset((file.id, file.path) for file in files)
        changeset_paths = frozenset(file.path for file in files)
        equiv_changeset_files = frozenset((file.id, file.path) for file in equiv_files)
        ...
        assert (
            changeset_paths == SINGLE_PATHLIST
...
async def root_changeset(phase, api, critic, repository):
    if phase == "pre":
        root_commit = await api.commit.fetch(repository, ref=ROOT_SHA1)
        try:
            await api.changeset.fetch(repository, single_commit=root_commit)
        except api.changeset.Delayed:
            pass

    elif phase == "post":
        root_commit = await api.commit.fetch(repository, ref=ROOT_SHA1)
        root_changeset = await api.changeset.fetch(
            repository, single_commit=root_commit
        )
        assert (
            root_changeset.type == "direct"
        ), "root_changeset should be direct changeset"
        assert isinstance(root_changeset.id, int), "root_changeset.id should be integer"
        root_paths = frozenset(
            [filechange.file.path for filechange in root_changeset.files]
        )

        assert (
            root_paths == ROOT_PATHLIST
...
async def bad_changesets(phase, api, critic, repository):
    if phase == "pre":
        params_list = [
            (None, None, None, None, AssertionError),
            (-5, None, None, None, api.changeset.InvalidId),
        ...
        for (
            ...
            try:
                if from_commit_ref is not None:
                    from_commit = await api.commit.fetch(
                        repository, ref=from_commit_ref
                    )
                else:
                    from_commit = None
                if to_commit_ref is not None:
                    to_commit = await api.commit.fetch(repository, ref=to_commit_ref)
                else:
                    to_commit = None
                if single_commit_ref is not None:
                    single_commit = await api.commit.fetch(
                ...
                assert False, "Invalid/missing parameters should raise exception"
    else:
        raise Exception
