...
def basic(arguments):
    ...
    branch = api.branch.fetch(critic, repository=repository, name=arguments.review)
    review = api.review.fetch(critic, branch=branch)
    alice = api.user.fetch(critic, name="alice")
    _bob = api.user.fetch(critic, name="bob")
    dave = api.user.fetch(critic, name="dave")
    _erin = api.user.fetch(critic, name="erin")
    ...
    comment_id_map = {comment.id: index for index, comment in enumerate(comments)}
