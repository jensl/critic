...
def basic():
    ...
    def fetch_review(number):
        branch = api.branch.fetch(
            critic, repository=repository, name="r/020-reviewrebase/%d" % number
        )
    ...
    check_commits(
        partition.commits,
        ["Test #1, commit 3", "Test #1, commit 2", "Test #1, commit 1"],
    )
    ...
    check_commits(
        partition.commits,
        ["Test #2, commit 3", "Test #2, commit 2", "Test #2, commit 1"],
    )
    assert partition.following is None
    ...
    check_commits(
        partition.commits,
        ["Test #3, commit 3", "Test #3, commit 2", "Test #3, commit 1"],
    )
    assert partition.following is None
    ...
    check_commits(
        partition.commits,
        [
            ("Merge branch '020-reviewrebase-4-1' " "into 020-reviewrebase-4-2"),
            "Test #4, commit 3",
            "Test #4, commit 2",
            "Test #4, commit 1",
        ],
    ...
    check_commits(
        partition.commits,
        ["Test #5, commit 3", "Test #5, commit 2", "Test #5, commit 1"],
    )
    assert partition.following is None
