...
def basic():
    ...
    def check_history_rewrite(rebase, old_head_summary, new_head_summary):
        ...
        assert rebase.branchupdate.to_head.summary == new_head_summary, (
            rebase.branchupdate.to_head.summary,
            new_head_summary,
        )

    def check_move_rebase(
        rebase,
        old_head_summary,
        new_head_summary,
        old_upstream_summary,
        ...
        assert all(
            ...
            for commit in (
                rebase.branchupdate.from_head,
                rebase.branchupdate.to_head,
                rebase.old_upstream,
                rebase.new_upstream,
            )
        )
    ...
    check_move_rebase(
        rebases[0],
        old_head_summary=(
            "Use temporary clones for relaying instead of " "temporary remotes"
        ),
        new_head_summary=(
            "Use temporary clones for relaying instead of " "temporary remotes"
        ),
