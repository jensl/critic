...
def basic():
    ...
    assert isinstance(commit.parents[0], api.commit.Commit), type(commit.parents[0])
    ...
    assert isinstance(commit.author, api.commit.Commit.UserAndTimestamp), type(
        commit.author
    )
    ...
    assert isinstance(commit.author.timestamp, datetime.datetime), type(
        commit.author.timestamp
    )
    assert commit.author.timestamp == COMMIT_AUTHOR_TS, commit.author.timestamp

    assert isinstance(commit.committer, api.commit.Commit.UserAndTimestamp), type(
        commit.committer
    )
    assert isinstance(commit.committer.name, str), type(commit.committer.name)
    assert commit.committer.name == COMMIT_COMMITTER_NAME, commit.committer.name
    assert isinstance(commit.committer.email, str), type(commit.committer.email)
    assert commit.committer.email == COMMIT_COMMITTER_EMAIL, commit.committer.email
    assert isinstance(commit.committer.timestamp, datetime.datetime), type(
        commit.committer.timestamp
    )
    assert commit.committer.timestamp == COMMIT_COMMITTER_TS, commit.committer.timestamp

    try:
        api.commit.fetch(repository, commit_id=47114711)
    except api.commit.InvalidId:
        pass
