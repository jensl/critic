...
def extractResourceSummary(resource_class):
    lines = list(
        splitAndDeindentDocstring(resource_class, level=1, default="Undocumented")
    )
    try:
...
def copyParagraph(destination, source, as_definition=False, include_empty=None):
    ...
    if as_definition:
        paragraph = ["= " + paragraph[0]] + ["  " + line for line in paragraph[1:]]
    if include_empty:
        paragraph.append("")
...
def describeVersion():
    ...
    for path, summary in supported_resources:
        lines.extend(["? <code>/api/v1/[%s][%s]</code>" % (path, path), "= " + summary])
...
def describeResource(resource_path):
    ...
    for index in reversed(range(1, len(path_with_arguments))):
        path_with_arguments.insert(index, "<i>%s</i>" % placeholders[index - 1])
    path_with_arguments = "/".join(path_with_arguments)

    lines = [resource_class.name.capitalize(), "=" * len(resource_class.name), ""]
    ...
    if resource_class.single:
        ...
        if description is None:
            ...
            lines.append(
                "? <code>api/v1/%s/ARGUMENT[,ARGUMENT,...]</code>" % path_with_arguments
            )
            copyParagraph(lines, description, as_definition=True, include_empty=True)
    ...
    if alternative_paths:
        ...
        for alternative_path in alternative_paths:
            lines.append(
                "[%s]: /api/v1?describe=%s" % (alternative_path, alternative_path)
            )
        lines.append("")
    ...
    if subresources:
        ...
        for path, _ in subresources:
            lines.append("[%s]: /api/v1?describe=%s/%s" % (path, resource_path, path))
