...
from ..manifest import Manifest, ManifestError, FilterHookRole
from ..execute import ProcessException, ProcessFailure, executeProcess


async def signalExtensionTasksService():
    # FIXME
    await background.utils.wakeup("extensiontasks")
...
def listFilterHooks(db, user):
    ...
    for extension_id, version_id, version_sha1, is_universal in installs:
        if version_id is not None:
            ...
            if not any(isinstance(role, FilterHookRole) for role in manifest.roles):
                continue
...
def queueFilterHookEvent(db, filter_id, review, user, commits, file_ids):
    ...
    with db.updating_cursor(
        ...
        "extensionfilterhookfiles",
    ) as cursor:
        cursor.execute(
            """INSERT INTO extensionfilterhookevents
        ...
        cursor.executemany(
            """INSERT INTO extensionfilterhookcommits
                        (event, commit)
               VALUES (%s, %s)""",
        ...
        cursor.executemany(
            """INSERT INTO extensionfilterhookfiles
                        (event, file)
               VALUES (%s, %s)""",
...
-- critic"""
        ...
        commits_text = "\n           ".join(
            ('%s "%s"' % (commit.sha1[:8], commit.niceSummary()) for commit in commits)
        )
        ...
        body = body % {
            ...
            "filter.data": json.dumps(filter_data),
            "review.id": review.id,
        ...
        mailutils.sendMessage(
            recipients=list(recipients), subject="Failed: " + role.title, body=body
        )
