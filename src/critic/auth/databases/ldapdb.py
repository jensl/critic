...
def escaped(fields, fn):
    return {identifier: fn(value) for identifier, value in fields.items()}
...
class LDAP(auth.Database):
    ...
    def __isMemberOfGroup(self, connection, group, fields):
        ...
        result = connection.search_s(
            group["dn"], ldap.SCOPE_BASE, attrlist=[group["members_attribute"]]
        )
        if len(result) != 1:
    ...
    async def authenticate(self, critic, fields):
        ...
        if cached_data is not None:
            ...
            try:
                user = await api.user.fetch(critic, cached_user_id)
            except api.user.InvalidId:
                pass
            else:
                await critic.setActualUser(user)
                critic.setAuthenticationLabels(cached_authentication_labels)
                return

        connection = self.__startConnection()

        search_base = self.configuration["search_base"] % escaped(
            fields, ldap.dn.escape_dn_chars
        )
        search_filter = self.configuration["search_filter"] % escaped(
            fields, ldap.filter.escape_filter_chars
        )
        ...
        if self.configuration["create_user"]:
            attributes.extend(
                [
                    self.configuration["fullname_attribute"],
        ...
        try:
            connection.simple_bind_s(dn, fields[self.configuration["credentials"]])
        ...
        if username is None:
            raise auth.AuthenticationError("Configured username LDAP attribute missing")

        try:
            user = await api.user.fetch(critic, name=username)
            ...
            fullname = getAttribute(self.configuration["fullname_attribute"], username)
            email = getAttribute(self.configuration["email_attribute"])

            async with api.transaction.start(critic) as transaction:
                modifier = await transaction.createUser(
                    username, fullname, email, email_verified=None
                )
    ...
    def getAuthenticationLabels(self, user):
        ...
        if "require_groups" in self.configuration:
            for group in self.configuration["require_groups"]:
                if "label" in group and self.__isMemberOfGroup(
                    connection, group, fields
                ):
                    authentication_labels.add(group["label"])
