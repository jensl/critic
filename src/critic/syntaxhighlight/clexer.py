...
RE_CTOKENS_INCLUDE_WS = re.compile(
    rejoin(
        [
            CONFLICT_MARKER,
            IDENTIFIER,
...
def split(
    ...
    if include_ws:
        ...
        expression = RE_CTOKENS
    tokens = [match.group(0) for match in expression.finditer(input)]
    if include_comments:
        return tokens
    else:
        return list(filter(lambda token: not iscomment(token), tokens))
...
def tokenize(
    ...
    for token in tokens:
        if isinstance(token, Token):
            ...
            yield Token(token, filename, line, column)
...
def group(tokens: List[TokenOrString], groups: Optional[Mapping[str, str]] = None):
    ...
    for index, token in enumerate(tokens):
        if token in groups:
            ...
            raise CLexerException(
                "%d:%d: expected '%s', got '%s'" % (line, column, currentEnd, token)
            )
        else:
            currentList.append(token)
    ...
    if len(stack) > 1:
        ...
        raise CLexerException(
            "%d:%d: unmatched group opener '%s'" % (line, column, token)
        )
...
def group1(
    ...
    raise CLexerGroupingException(
        "unmatched group opener '%s'" % token, list(flatten(currentList))
    )
...
def partition(tokens: Iterable[str], separator: str) -> Sequence[Sequence[str]]:
    ...
    try:
        ...
        if len(partitions) == 1 and not partitions[0]:
            return []
        else:
            return partitions
...
def join(tokens: NestedTokenList, insertSpaces: bool = True) -> str:
    if insertSpaces:
        ...
        for token in flatten(tokens):
            if not lastWasSpace and not token.isspace():
                result += " "
            result += str(token)
...
if __name__ == "__main__":
    ...
    def testToken(
        ...
        for other in SUBPATTERNS:
            if other != subpattern and not (
                isOperator
                and other == PREPROCESSOR_DIRECTIVE
                and (token == "#" or token == "##")
            ):
