...
def sendParentFiltersApplied(db, from_user, to_user, review, assigned):
    # First check if the user has activated email sending at all.
    if not to_user.getPreference(db, "email.activated"): return []
...
""" % data
    ...
    text = "%s has modified the assignments in the review by making filters from upstream repositories apply, which they previously did not.  This had the effect that you are now a %s the review." % (from_user.fullname, "reviewer of changes in" if assigned else "watcher of")
