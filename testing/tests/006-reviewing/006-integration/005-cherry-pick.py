...
def test2():
    ...
    with frontend.signin("alice"):
        ...
        testing.expect.equal(expected=1, actual=len(review_commits))
...
def test3():
    with repository.workcopy(clone="small") as workcopy:
        ...
        review_3.addFile(nonsense="nonsense3.txt")
        review_3.commit("Add some nonsense\n", nonsense=nonsense("first"))
        review_3.commit("Add more nonsense\n", nonsense=nonsense("second"))
    ...
    with frontend.signin("alice"):
        ...
        frontend.json(
            ...
            expect={
                ...
                "successful": False,
                "error_message": "review branch must contain a single commit",
            },
        )
...
def test4():
    with repository.workcopy(clone="small") as workcopy:
        review_4 = Review(workcopy, "alice", f"{test.name}/4")
        review_4.addFilter("bob", "reviewer")
        review_4.addFile(conflict="nonsense1.txt", nonsense="nonsense4.txt")
        conflicting_sha1 = review_4.commit(
            "Add conflicting nonsense\n",
            conflict=nonsense("conflict"),
            nonsense=nonsense("first"),
        )
        review_4.target_branch = target_branch
        review_4.submit()

    markAllAsReviewed(review_4)

    with frontend.signin("alice"):
        integration_id = frontend.json(
            "reviewintegrationrequests",
            post={"review": review_4.id},
            expect={
                "id": int,
                "review": review_4.id,
                "target_branch": target_branch["id"],
