...
with repository.workcopy(clone="small") as work:
    ...
    review.addFile(
        first="first.txt", second="second.txt", third="third.txt",
    )
    review.commit(
        "Reference commit",
        reference=True,
        first=["First", "=====", "Initial line"],
        second=["Second", "======", "Initial line"],
        third=["Third", "=====", "Initial line"],
    )
    review.commit(
        "First commit", first=["First", "=====", "Initial line", "Added line"]
    )
    review.commit(
        "Second commit", second=["Second", "======", "Initial line", "Added line"]
    )
    review.commit(
        "Third commit", third=["Third", "=====", "Initial line", "Added line"]
    )
    ...
    changesets = {
        "first": fetch_changeset(
            {"from": review.sha1s[0], "to": review.sha1s[1]}, repository="small"
        ),
        "second": fetch_changeset(
            {"from": review.sha1s[1], "to": review.sha1s[2]}, repository="small"
        ),
        "third": fetch_changeset(
            {"from": review.sha1s[2], "to": review.sha1s[3]}, repository="small"
        ),
        "all": fetch_changeset(
            {"from": review.sha1s[0], "to": review.sha1s[3]}, repository="small"
        ),
    }

    issues = {"alice": [], "bob": [], "dave": []}
    ...
    with frontend.signin("alice"):
        ...
        frontend.json(
            ...
            params={"unpublished": "yes"},
            expect=batch_json(
                review.id, "alice", "draft", created_comments=[issues["alice"][0]]
            ),
        )
        ...
        frontend.json(
            ...
            expect=batch_json(
                ...
                "alice",
                "draft",
                created_comments=[issues["alice"][0], issues["alice"][1]],
            ),
        )
    ...
    with frontend.signin("bob"):
        ...
        frontend.json(
            ...
            put={"draft_changes": {"new_state": "resolved"}},
            expect={
                "id": issues["alice"][0],
                "state": "open",
                "draft_changes": comment_draft_changes_json(
    ...
    with frontend.signin("dave"):
        ...
        frontend.json(
            "reviewablefilechanges/%d,%d"
            % (changes["second"][0]["id"], changes["third"][0]["id"]),
            put={"draft_changes": {"new_is_reviewed": True}},
            expect={
                "reviewablefilechanges": [
                    {
        ...
        frontend.json(
            ...
            expect=batch_json(
                ...
                resolved_issues=[issues["alice"][0], issues["alice"][1]],
