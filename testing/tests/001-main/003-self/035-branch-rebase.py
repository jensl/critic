...
FILENAME = "035-branch-rebase.txt"
...
with repository.workcopy() as work:
    ...
    work.run(["checkout", "-b", "035-branch-rebase/1", "origin/master"])

    commits_1 = [commit("1/first"),
                 commit("1/second"),
                 commit("1/third")]

    work.run(["push", instance.repository_url("alice"), "HEAD"])

    work.run(["checkout", "-b", "035-branch-rebase/2", commits_1[1]])

    commits_2 = [commit("2/first"),
                 commit("2/second"),
                 commit("2/third")]
...
def branch_sha1s(branch_name):
    commits = frontend.json(
        "branches/%d/commits" % branch_id(branch_name)
    )["commits"]
    return set(commit["sha1"] for commit in commits)

testing.expect.equal(set(commits_1), branch_sha1s("035-branch-rebase/1"))
testing.expect.equal(set(commits_2), branch_sha1s("035-branch-rebase/2"))
...
with frontend.signin("alice"):
    frontend.operation(
        ...
        data={
            "repository": "critic",
            "branch_name": "035-branch-rebase/2",
            "base_branch_name": "master"
        })

testing.expect.equal(set(commits_1),
                     branch_sha1s("035-branch-rebase/1"))
testing.expect.equal(set(commits_1[:2] + commits_2),
                     branch_sha1s("035-branch-rebase/2"))

def commit_sha1s(result, commit_ids):
    sha1_by_id = {
        commit["id"]: commit["sha1"]
        for commit in result["linked"]["commits"]
    }
    return [sha1_by_id[commit_id] for commit_id in commit_ids]

result = frontend.json(
    "branches/%d/branchupdates" % branch_id("035-branch-rebase/2"),
    params={
        "include": "commits",
    }
)
...
testing.expect.equal(
    set(commits_2),
    set(commit_sha1s(result, result["branchupdates"][0]["associated"])))
...
with frontend.signin("alice"):
    frontend.operation(
        ...
        data={
            "repository": "critic",
            "branch_name": "035-branch-rebase/2",
            "base_branch_name": "035-branch-rebase/1"
        })
...
with frontend.signin("alice"):
    frontend.operation(
        ...
        data={
            "repository": "critic",
            "branch_name": "035-branch-rebase/1",
            "base_branch_name": "035-branch-rebase/2"
        })

result = frontend.json(
    "branches/%d/branchupdates" % branch_id("035-branch-rebase/1"),
    params={
        "include": "commits",
    }
)
