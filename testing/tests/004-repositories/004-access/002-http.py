...
def git(args, cwd=None):
    ...
    output = subprocess.check_output(
        argv, cwd=cwd, env=environ, stderr=subprocess.STDOUT, encoding="utf-8"
    )
...
try:
    ...
    try:
        git(
            ["push", "--quiet", "origin", "HEAD:007-http-backend-1"],
    ...
    try:
        git(
            ["push", "--quiet", "origin", "HEAD:007-http-backend-2"],
            cwd=os.path.join(work_dir, "critic"),
        )
    except subprocess.CalledProcessError as error:
...
try:
    ...
    try:
        git(
            ["push", "--quiet", "origin", "HEAD:007-http-backend-3"],
            cwd=os.path.join(work_dir, "critic"),
        )
        logger.error("Unauthenticated push (apparently) accepted!")
    except subprocess.CalledProcessError:
    ...
    try:
        git(
            ["push", "--quiet", "origin", "HEAD:007-http-backend-4"],
            cwd=os.path.join(work_dir, "critic"),
        )
    except subprocess.CalledProcessError as error:
        logger.error("'git push' failed: %s\n%s" % (str(error), error.output.rstrip()))
